...
def computeDelta(wt, X, Xi):
    ...
    def similarity(a, b):
        ...
        std_b = np.std(b[:-1])
        for i in range(0, M):
            sim += ((a[i] - mean_a) * (b[i] - mean_b))
        sim /= (M * std_a * std_b)
        return sim
    predict, numerator, denominator = 0.0, 0.0, 0.0
    for index, row in Xi.iterrows():
        yi = row[-1]
        numerator += yi * math.exp(wt * similarity(X, row))
...
# Use the variable name model for fitted model
